...
#in directory /scratch/Users/sasp9297/CLASS_2020/analysis/00_class_exercises/lncrna_group
```
...
probably not, would lead to high false discovery rate (FDR), try to only place where it's reasonable 
...
Varience test to normalize for the disribution so it's more representative of actual distribution
...
#making list of chromosomes and lengths
  chrLens <- genome[names(seqlengths(peak_gr)),"size"]
#making a vector of which chromosome each is on 
  nn <- as.vector(seqnames(peak_gr))
#ordering by chromosome number
  ii <- order(nn)
#taking widths of the peaks
  w <- width(peak_gr)
#table of which chromosome each is on 
  nnt <- table(nn)
#ordering the table by the chromosome number 
  jj <- order(names(nnt))
#no idea, redundant? 
  nnt <- nnt[jj]
#no idea, redundant?
...
#do I need to run for all or should I just do it for 3?
...
inflation_rate <- as.numeric(object.size(permuted_peaks))/as.numeric(object.size(peak_list[1:4]))
...
#don't run all peaks, just run 3 bc it takes a long time, could run in R session on the terminal to be faster
set.seed(12343)
...
#all enrichment is greater in the mRNA promoters than the lncRNA promoters, some more contrasting than others (i.e. ARID1B >AFF1)
